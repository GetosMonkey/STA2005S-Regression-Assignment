...
for (i in 1:2){
  ...
  if (i<0.001){ stars[i] = "***" } 
  else if (i < 0.01){ stars[i] = '**' }
  else if (i < 0.05){ stars[i] = '*' }
  else if (i < 0.1) { stars[i] = "."}
}
...
fit <- lm(consumption_kwh ~ 
            outside_temperature + humidity + wind_speed + household_size + appliance_index + energy_efficiency + solar_installed + day_of_week + holiday +
